...
public abstract class AbsPreviewImageActivity extends AppCompatActivity implements ViewPager.OnPageChangeListener{
    public static final String ARG_INT_LAYOUT_ID ="LAYOUT_ID";
    public static final String ARG_LIST_STR_IMAGES ="IMAGES";
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        int layoutId=getIntent().getIntExtra(ARG_INT_LAYOUT_ID,R.layout.act_preview_image); //使用自定义或者默认布局
        setContentView(layoutId);

        mViewPager=(ViewPager)findViewById(R.id.viewPager);
        mIndicator=(TextView)findViewById(R.id.indicator);
        mData=getIntent().getStringArrayListExtra(ARG_LIST_STR_IMAGES);
        int index=getIntent().getIntExtra(ARG_INT_INDEX,0);
