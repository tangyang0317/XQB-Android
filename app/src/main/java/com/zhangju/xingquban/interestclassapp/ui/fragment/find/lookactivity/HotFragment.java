...
import com.zhangju.xingquban.interestclassapp.ui.fragment.find.lookactivity.adapter.LookListAdapter;
...
public class HotFragment extends FastFragment {
    @BindView(R.id.recyclerview_look_activity)
    RecyclerView recyclerviewLookActivity;
    Unbinder unbinder;

    private List<LooklistDataBean.AaDataBean> mLookList = new ArrayList<>();
    ...
    private void setLookHotAdapter() {

    }

    private void getLookHotList() {
        final Request request = Request.obtain(LookActiviInterface.POST_LOOK_HOT);
        ...
        request.setListener(new SimpleListener<LooklistDataBean>() {
            ...
            public void onResponseListener(Request r, LooklistDataBean result) {
                ...
                try {
                    if (success) {
                        List<LooklistDataBean.AaDataBean> dataBean = result.getAaData();
